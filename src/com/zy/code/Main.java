...
//    leetcode78 子集
    ...
    private void dfs(char[][] grid,int i,int j){
        if (!numIslandsArea(grid, i, j)) return;
    ...
    private Boolean numIslandsArea(char[][] grid,int i,int j){
    ...
    public boolean isPalindrome(ListNode head) {
        ...
        ListNode secondHalfStart = isPalindrome1(firstHalfEnd.next);
        ...
        // 还原链表并返回结果
//        firstHalfEnd.next = isPalindrome1(secondHalfStart);
        return result;
    }
    ...
    private ListNode isPalindrome1(ListNode head){
        ListNode pp = null,cur = head,newhead = null,next = null;
        while (cur != null){
            if (cur.next == null){
                newhead = cur;
            }
...
//# 只要用相同的方法 分别处理  左右数组就可以了
    ...
    public int ZeroOnePack2(int V,int N,int[] weight,int[] value){
        int[] dp = new int[V+1];
        for (int i = 1; i<N; i++){
            for (int j = V; j>weight[i-1]; j--){
                dp[j] = Math.max(dp[j-weight[i-1]]+value[i],dp[j]);
            }
        }
        return dp[V];
    }
    ...
    public String completePack(int V,int N,int[] weight,int[] value){
        int[][] dp = new int[N+1][V+1];
        for (int i = 1; i<N+1; i++){
            for (int j = 1;j<V+1; j++){
                if (weight[i-1] > j){
